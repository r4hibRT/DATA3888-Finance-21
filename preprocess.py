...
BUCKET_SIZE   = 120    # seconds per bucket
N_BUCKETS     = 4      # 4 × 120s = 480s input window
INPUT_SECONDS = BUCKET_SIZE * N_BUCKETS  # 480
TARGET_START  = 480    # seconds 480–719 used for target RV

VAL_RATIO   = 0.1      # fraction of train time_ids held out for validation
RANDOM_SEED = 42
RV_FLOOR    = 1e-8     # floor for TARGET log(RV) only
N_WORKERS   = 16        # parallel workers (tune to your CPU)

# Feature names in order — 4 per bucket, 4 buckets = 16 total
BUCKET_FEATURE_NAMES = [
    "log_return",
    "realized_vol",
    "mean_spread",
    "total_volume",
    "mean_price_spread",        # NEW
    "mean_volume_imbalance",    # NEW
]
...
]   # ['log_return_b0', 'realized_vol_b0', ..., 'total_volume_b3']

INPUT_DIM = len(FEATURE_COLS)   # 16
...
def bucket_features(bucket_df: pd.DataFrame) -> dict[str, float]:
    ...
    price_spread     = bucket_df["price_spread"].to_numpy(dtype=np.float64)
    vol_imbalance    = bucket_df["volume_imbalance"].to_numpy(dtype=np.float64)

    # 5. mean_price_spread
    valid_ps = price_spread[np.isfinite(price_spread) & (price_spread > 0)]
    mean_price_spread = float(np.mean(valid_ps)) if len(valid_ps) > 0 else 0.0

    # 6. mean_volume_imbalance  (can be negative, so no >0 filter)
    valid_vi = vol_imbalance[np.isfinite(vol_imbalance)]
    mean_volume_imbalance = float(np.mean(valid_vi)) if len(valid_vi) > 0 else 0.0
    ...
    return {
        ...
        "mean_price_spread"    : mean_price_spread,       # NEW
        "mean_volume_imbalance": mean_volume_imbalance,   # NEW
    }
